...
**Rung 2G — Doom-style nozzle + water-jet projectile system.** *Added 2026-04-17, revised 2026-04-17 per `docs/ADR-001-PRESSURE_WASHER_FPS_WEAPON.md`.* Equip Gleaner with a first-person nozzle sprite (three-state: absent / stowed / brandished) in the screen-space sprite slot already wired in `raycast.js-master/r_draw.js:195-224`. Adapt `EyesOnly/public/js/projectile-system.js` as `engine/pw-projectile.js` — **retain ricochet** (water blobs arc, bounce, paint on every impact — Super Mario Sunshine FLUDD feel), add per-frame `gravityY: 0.08` accumulation on `vy`, cap 6 active jets, 120ms fire-rate cooldown. Splatter writes into `HoseDecal._paintStripe` on every bounce via the spec's max-combine additive semantics. Combat initiation routes through `CardAuthority.resolveProjectileHit({target, source, floorId, impact, fallback: 'knockback_only'})` — **no per-shot card commit**; ammo is hose energy (`SPRAY_DRAIN_MULT = 3.0`) applied on top of existing `HoseState` drain. Brandish toggle is a tap-button in a new `'brandish'` mode on `NchWidget` (joker stack compacts → water-gun button appears when hose is attached + not in combat). **Anchor interaction: "shoot the torch lights out"** routes projectile wall-hits at depth ≥ 3 into the already-shipped `TorchHitResolver.onHoseHit(floorId, cx, cy)` — no new `Lighting.extinguishTorch` method, no invented stealth cone (that's a future ADR). Kink readout is **gun jitter** via `jitterAmp = min(6, kinks * 1.8)` on the sprite draw offset. New modules: `pw-nozzle.js` (Layer 2, ~4h), `pw-projectile.js` (Layer 1, ~4h), `pw-fire-controller.js` (Layer 3, ~1h), plus `NchWidget` brandish-mode wiring (~1h). Est. **~14h ≈ 1.75 dev days**. Lands between 2F and 8 so the splatter paints into a decal system already working in 3D. See ADR §Action Items for the full task list.
...
**Validator primitive — edge-passage bitmap (prior art in `dcexjam2025`).** The constraint check above needs a fast "has this candidate hose route touched this edge?" query for every edge on the grid. `dcexjam2025/src/common/crawler_state.ts` already solves this shape of problem (for its secret-passage memory) with two bit flags per cell — `VIS_PASSED_EAST = 2` and `VIS_PASSED_NORTH = 4`. Each edge is canonically owned by exactly one cell (the cell whose EAST or NORTH wall the edge is), with stamping in `crawler_controller.ts:1745-1777` writing both the crossed cell's bit and the neighbor's mirror bit on a move. Full-grid edge state is 2 bits × cell count — *independent of path length or number of crossings*. Adopt the same encoding for the 2F validator's scratchpad: a packed Uint8Array a generator can stamp candidate routes into, run the contract check over, and discard in O(width × height). Strictly weaker than `HoseDecal`'s per-visit ledger — it drops ordering and entry/exit directions — but that's exactly what the contract check wants: "touched yes/no" in O(1) per edge. `HoseDecal` itself keeps the richer ledger because runtime gameplay needs ordering (for retraction) and entry/exit (for stripe rendering); the bitmap is a purpose-built generation-time tool, not a replacement for the ledger.
...
| 2G | Nozzle sprite + water-jet projectile + NchWidget brandish mode (ADR-001) | ~14h / 1.75 days | Yes (sprite slot only) | Perf ≤0.3ms combined on Hero's Wake B2 with 6 active jets; torch-extinguish routes into `TorchHitResolver.onHoseHit`; brandish toggle flips `NchWidget` to 3rd mode; card-fan collapse on brandish works clean; gun jitter scales with `HoseState.getKinkCount()` |

**A + B ship first as a pair** because they're the cheapest validation of the whole design — if the minimap stripes don't read, the 3D decal won't either. Everything after C is gated on perf, then on playtest, then on generator tuning. The PWS-pillar rungs (audio, sub-target, gyro) resume after 2F at the new Rung 3 position. **Rung 2G (nozzle + projectile) slots between 2F and Rung 8** so splatter paints into a decal system already validated in 3D — see `docs/ADR-001-PRESSURE_WASHER_FPS_WEAPON.md` for module layering, state machine, and the full 10-item action list.
