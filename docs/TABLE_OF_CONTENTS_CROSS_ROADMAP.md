...
| DOC-115 | [TILE_TEXTURE_HANDOFF.md](#doc-115-tile_texture_handoffmd) — Tile texture & architectural shape handoff. Gap matrix for tiles 40–54: infrastructure tiles (40–48) have textures, creature tiles (49–54) need procedural generators. Biome palette assignments, spatial contract entries, visual style guidance per tile. **Biome wiring shipped 2026-04-17** (tiles 45–47 + 49–54 added to biome-map.json). Creature textures pending. | docs/ |
| DOC-116 | [GATE_TAXONOMY.md](#doc-116-gate_taxonomymd) — **Gate taxonomy & resolution pipeline.** Six universal gate types (key, quest, faction, schedule, breakable, composite) with tile-agnostic `gates` map. Three-tier resolution: tile override → edge gate → floor gate → no gate. `override: true` flag, no partial inheritance. Stateful vs stateless evaluation model. Tooling surface: World Designer edge gate editor, BV meta panel gate inspector, CLI `bo set-gate`/`bo set-edge-gate`/`bo validate-gates`. Migration path from `lockedDoors`. Delegation boundaries for engine/UI agents. Depends on DOC-107 (reputation), DOC-105 (blockout refresh). **Quest-side coordination slice shipped 2026-04-17** — `QuestTypes.WAYPOINT_KIND.GATE_OPENED`, `QuestChain.onGateOpened(floorId,x,y,gateType)` + `kind:'gate-opened'` predicate (floorId/x/y/gateType optional AND-filters), `QuestChain.isStepComplete(questId, stepIdxOrId)` (int/string addressed), `QuestRegistry.flagReferenced(flag)` + `hasStep(questId, stepIdxOrId)` for `bo validate-gates`; 54/54 fresh-inode Node harness assertions pass (`tools/_doc116-cache/verify-gate-coord.js`). §8a.4 QUEST/FACTION gates now UNBLOCKED for authoring; engine-side `evaluateGate()` + `onGateUnlock()` + `lockedDoors→gates` migration still pending. | docs/ |
| DOC-117 | [ADJACENT_TILE_DECOR_SPEC.md](#doc-117-adjacent_tile_decor_specmd) — **Adjacent-tile decor system.** Data-driven adjacent-decor spawner: AdjacentDecorMap (Layer 1 registry keyed by source tile ID) + AdjacentDecorSpawner (Layer 3 subscriber on `FloorManager 'floor-loaded'` writing floor/wall overlays into Raycaster via `setFloorDecor`/`mergeWallDecor`). Three tiers: Tier 1 = creature-trace decor (19 sprites: feathers, bones, scratches, fungal spread, conduit infra, blood); Tier 2 = botanical (SHRUB/TREE); Tier 3 = infrastructure overgrowth (FENCE/PILLAR/WATER/BONFIRE/DETRITUS). `neighborTiles` filter + `directions` modes (cardinal / facing-tile / inline / front-1tile). Depends on DOC-115 (tile textures). **Tier 1 scaffolding shipped 2026-04-17** — AdjacentDecorMap + AdjacentDecorSpawner IIFEs + FloorManager event + Raycaster sink wired, runtime no-op until Tier 1 sprites land. Sprite taxonomy (DOC-117a) authored; six primitive helpers + 19 sprite generators pending. | docs/ |
| DOC-117a | [DOC-117_TIER1_SPRITE_TAXONOMY.md](#doc-117a-doc-117_tier1_sprite_taxonomymd) — **DOC-117 Tier 1 sprite taxonomy addendum.** Design contract for the 19 Tier 1 adjacent-decor sprites. Six procedural archetypes (Scatter / Slash / Streak / Blob / Thread / Plate) → six pixel-test helper primitives (`_pxScatter`, `_pxSlash`, `_pxStreak`, `_pxBlob`, `_pxThread`, `_pxPlate`) that live in `engine/texture-atlas.js` alongside `_createTexture`/`_hash`/`_clamp`. Each sprite composes over primitives in 10–30 lines. Worked examples (feather, scratch, fungus-with-thread). Six-batch implementation sequencing (primitives → scatter ×8 → slash ×4 → blob ×3 → streak ×2 → singletons ×2 → AdjacentDecorMap wiring). Blocks DOC-117 §8a step 2. | docs/ |
...
| DOC-118 | [CLI_TOOLS.md](#doc-118-cli_toolsmd) — **Canonical CLI inventory.** Every shell-invocable tool the project ships or depends on (local serve, extract-floors and siblings, `bo` / `npc` authoring CLIs, sidecar generators, validators, test harnesses, smoke scripts, webOS build scripts, code-review-graph MCP builders, raycast.js-master npm scripts). Uses `<repo-root>` placeholder convention so instructions are environment-agnostic. Linked from README / CLAUDE.md / AGENTS.md as the single source of truth for tool usage. Authored 2026-04-19 alongside the fresh-terminal / env-agnostic doc refresh. | docs/ |
| DOC-119 | [CLI_CONSOLIDATION.md](#doc-119-cli_consolidationmd) — **Tools dir cleanup memo.** Inventories the 11 `tools/_*-cache/` phase-scratch dirs (~700 KB), the v2/scratch stubs (`boxforge-cli-v2.js`, `extract-floors-v2.js`, `extract-floors.js.old`), and the `.bak.css-extract` / `.fixed` visualizer snapshots. Recommends archival layout under `tools/_archive/` with move commands. Cross-references DOC-118. Additive-only: no files were moved/deleted during authoring. | docs/ |
| DOC-120 | justfile — **Task runner** at `<repo-root>/justfile`. Wraps canonical commands from CLI_TOOLS.md into `just serve`, `just extract-all`, `just validate-all`, `just bo`, `just npc`, `just pre-commit`, `just pre-serve`, `just post-blockout-edit`, `just fresh-clone`. Works on Windows/macOS/Linux once `just` (https://just.systems) is installed. | root/ |
| DOC-121 | scripts/dg-profile.{sh,ps1} — **Shell profile snippets.** Source into `~/.bashrc` or `$PROFILE` to get `bo` / `npc` / `boxforge` / `dg-serve` / `dg-extract-all` / `dg-validate` / `dg-pre-*` / `dg-post-blockout-edit` / `dg-fresh-clone` / `dg-versions` functions. Honors `$DG_REPO_ROOT` so functions work from anywhere. | scripts/ |
| DOC-122 | [scripts/workflow/README.md](#doc-122-scriptsworkflowreadmemd) — **Standalone workflow macro wrappers.** `dg-fresh-clone`, `dg-pre-serve`, `dg-pre-commit`, `dg-post-blockout-edit` in bash + PowerShell. Each wrapper sources `dg-profile` then calls the function, so logic stays single-source. Usable without sourcing the profile. | scripts/workflow/ |
| DOC-123 | [EOL_DECISION.md](#doc-123-eol_decisionmd) — **Line-ending configuration decision memo.** Reassesses the old `core.autocrlf=false` / `core.safecrlf=warn` / `core.eol=lf` pinning on the fresh terminal. Documents current state (all three unset locally and globally), risk surfaces that remain (fresh clones, shebang scripts, content-hash sensitivity), and recommends Option A (local repin now). Includes rollback commands. | docs/ |
| DOC-124 | [BOXFORGE_README.md](#doc-124-boxforge_readmemd) — **BoxForge v2.0 product README.** Zero-dependency CSS-3D box geometry editor. Three-phase animation (Idle/Hover/Activated), glow sources, orb/pyramid composers, project persistence as `.boxforge.json`. Migrated from `tools/BF README.md` on 2026-04-19. | docs/ |
| DOC-125 | [BLOCKOUT_VISUALIZER_README.md](#doc-125-blockout_visualizer_readmemd) — **Blockout Visualizer README.** Canvas-based tile editor for floor grids; v0.15 (Apr 2026) covers Tier 1 draw/select/history, Tier 2 direct file writes + validation, Tier 3 per-floor metadata editor, Tier 4 window-scene editor, Tier 6 Passes 1–4 (`window.BO.run()` headless surface, CLI wrapper, perception toolkit, stamp library). Migrated from `tools/BO-V README.md` on 2026-04-19. | docs/ |
| DOC-126 | [BLOCKOUT_VISUALIZER_AGENT_FEEDBACK.md](#doc-126-blockout_visualizer_agent_feedbackmd) — **Agent feedback notes for the Blockout Visualizer.** Short operational notes captured during agent authoring sessions (e.g., Floor 3.1.1 Sealab Depths learnings). Migrated from `tools/BO-V agent feedback.md` on 2026-04-19. | docs/ |
| DOC-127 | [BLOCKOUT_VISUALIZER_ROADMAP_V2.md](#doc-127-blockout_visualizer_roadmap_v2md) — **Blockout Visualizer engine roadmap v2.** Long-form roadmap document (~87 KB) covering the full BO-V feature plan — tiers, passes, data contracts, harness coverage. Migrated from `tools/BLOCKOUT_VISUALIZER_ROADMAPv2.md` on 2026-04-19. Supersedes `docs/Archive/BLOCKOUT_VISUALIZER_OLD.md`. | docs/ |
| DOC-128 | [TOOLS_SHORT_ROADMAP.md](#doc-128-tools_short_roadmapmd) — **Active-tracks scratchpad for tooling work.** Three tracks tracked in-flight: BO-V tooling pass (near-complete), Seed / Save-Load / World-Designer (M1/M2 active, M3 parked), Agent-feedback closeouts (Track C). Includes the `Math.random` audit and M2.5 `buildVersion` gate notes referenced by `engine/save-state.js`. Migrated from `tools/short-roadmap.md` on 2026-04-19. | docs/ |
| DOC-129 | [BUILD_VERSION_POLICY.md](#doc-129-build_version_policymd) — **Playtester-facing build version stamp.** Policy for reporting which commit a tester played. `scripts/stamp-build.sh` / `.ps1` regenerates `engine/game-build-stamp.js` from the current git state; the stamp exposes `window.GameBuildStamp.display` (e.g. `"build 0.14.2 · a1b2c3d · 2026-04-19"`) which playtesters quote verbatim. Wired into `justfile` (`just stamp-build` / `just stamp-and-commit`) and `dg-profile` (`dg-stamp-build`). Complements the existing `SaveState.BUILD_VERSION` save-schema gate. Authored 2026-04-19. | docs/ |
| DOC-130 | [SPRINT_PLAN_VOTING_PERIOD_PATCH.md](#doc-130-sprint_plan_voting_period_patchmd) — **Voting-Period Patch sprint plan.** 2026-04-20 → 2026-04-27 with ship gate Sat 2026-04-25. Active patch-sprint roster: Foxtrot.BlockoutRefreshAndFloorUpdate (Phases B–D of `BLOCKOUT_REFRESH_PLAN.docx` + per-floor §7 updates across floors 0–3.1.N), Golf.UiUnificationAndRefillAudit (unify `fill`/`restock`/`refuel`/`grab_hose` verb + icon per peer feedback), Hotel.TutorialInteractionPolish (pressure wash / trap rearm / cobweb / crate / corpse / torch end-to-end playability), India.PatchBuildGate (Day-5 stamp + release). Stretch Juliet.BlockoutRefreshPhaseE (Boss Door) gated on Foxtrot landing B–D by end-of-Wed. Original Alpha–Echo roster paused-not-cancelled; patch-sprint sections are additive. Day-by-day dispatch schedule, Definition of Done per section, risks, and open Day-1 questions enumerated. Authored 2026-04-20. | docs/ |
...
| DOC-109 | [DEBRIEF_FEED_CATEGORIES_ROADMAP.md](#doc-109-debrief_feed_categories_roadmapmd) — **HUD debrief-feed category wrapper (Roadmap A).** Two expandable categories (Readiness, Relationships) collapsing 3 dungeon readiness bars + 4 factions + N NPCs into EyesOnly-style rows where most-recently-updated subject stays visible when collapsed and expansion auto-retracts on new activity. Extends ReputationBar with `subject:id` namespacing for NPC favor, adds ReadinessCalc event bus, migrates existing per-faction `expanded` flags. **Phases 0 + 1 + 2 + 3 + 4 + 5 shipped 2026-04-17** — Phase 0: ReputationBar subject-kind namespace (`faction:id`/`npc:id`), 51/51 harness green, Function.length-based 3-arg back-compat. Phase 1: ReadinessCalc event bus (`on`/`off`/`markDirty`/`getGroupScore`/`invalidate`, microtask-debounced via `Promise.resolve().then`, DungeonSchedule group aggregation), 32/32 harness green, **including core-feeder callsite wiring** — `_markDirty(floorId)` installed in all 5 CORE_FEEDERS (cleaning-system, torch-state, trap-rearm, cobweb-system, crate-system) at every state-mutation site plus `invalidate()` on global `reset()`/`clearAll()`. Secondary feeders (breakable-spawner, spray-system, corpse-actions, torch-ring) deferred to Phase 3 audit. Phase 2: DebriefFeed `_categories` wrapper scaffold — readiness + relationships categories with `revealCategory`/`expandCategory`/`collapseCategory`/`toggleCategory`/`updateReadiness`/`updateRelationship` public surface; faction rows migrated under `'faction:<id>'` row-ID namespace; legacy `expandFaction`/`collapseFaction`/`updateFaction`/`getFactionState` delegate through `_categories.relationships`; 37/37 harness green (G1: 11, G2: 6, G3: 6, G4: 9, G5: 5) via `verify-phase2-final.js` fresh-inode mirror pattern. Phase 3: DebriefFeed readiness category wires — `GROUP_DATA` display-side table (spade/club/diamond → Coral Cellars ♠ / Hero's Wake ♣ / Ironhold Depths ♦ with biome tints reusing FACTION_COLORS), `_setReadinessRow` state mutator + `_readinessRow` renderer with ★ overheal accent past 100% + `width:100%` visual clamp, `updateReadiness(groupId, coreScore, meta)` / `getReadinessState(groupId)` public API with sticky reveal gate, readiness block rendered ABOVE relationships in `_renderUnified`, Game.init subscribes `ReadinessCalc.on('group-score-change')` → `DebriefFeed.updateReadiness` + `ReadinessCalc.invalidate()` seeds initial bars, `verify-phase3.js` 24/24 harness green (G1: 5, G2: 5, G3: 4, G4: 6, G5: 4) via fresh-inode mirror `tools/_readiness-phase3-cache/_fresh-debrief-feed.js`, Phase 0/1/2 + DOC-113 Phase C (sprint timer) regressions all re-green on Phase-3 mirror (228/228 total across five harness suites). Phase 4: DebriefFeed relationships category finalized — `updateRelationship(kind, subjectId, favor, tier, meta)` + `getRelationshipState(kind, subjectId)` unified public API with meta-bag persistence (icon/name/factionId/floor merged field-by-field across favor-change calls) and `flair.tierCrossed` routing via top-level `meta.tierCrossed=true`, `_npcRow` renderer with portrait glyph + display name + faction-tinted progress bar + reveal/bump/tier-cross keyframe juice, `_renderRowByKind` kind-dispatch now wires `'npc'` to `_npcRow` alongside `'faction'` and `'readiness'`, NpcSystem.getNpcMeta cross-floor lookup (`{id, name, emoji, factionId, floorId}` from the registered defs) powers Game.init's ReputationBar→DebriefFeed fan-out (subscribes both `favor-change` + `tier-cross` events, resolves NPC meta lazily, forwards tier-crossed flair via `meta.tierCrossed=true`), DispatcherChoreography.init now takes `onComplete(firstTime)` so the first-BPRD encounter reveals the relationships category + grants +100 favor to cascade a tier-cross animation, `index.html` adds `.df-npc-row` / `.df-npc-head` / `.df-npc-icon` / `.df-npc-name` / `.df-npc-tier` / `.df-npc-track` / `.df-npc-fill` CSS + `@keyframes df-npc-reveal/df-npc-bump/df-npc-tiercross` mirroring the faction-row vocabulary, `verify-phase4-v2.js` 22/22 harness green across 6 groups (G1 API surface, G2 faction migration back-compat, G3 NPC meta persistence + partial-merge, G4 flag routing justRevealed/justBumped/justTierCrossed, G5 sticky reveal-gate, G6 render DOM dispatch), Phase 0/1/2/3 + DOC-113 Phase C regressions all re-green on Phase-4 mirror. Phase 5: Auto-retract on activity — `CATEGORY_MIN_EXPAND_WINDOW_MS=600`ms grace window + `CATEGORY_RETRACT_DELAY_MS=600`ms debounced collapse delay + `expandedAtTs` anchor stamped on expand (idempotent expand does not re-stamp) + `_maybeScheduleRetract(catId)` gate called from every `updateReadiness`/`updateRelationship` (no-op if cat is unexpanded or inside grace window) + `_scheduleRetract`/`_cancelRetract` via `setTimeout`-debounced `_retractTimers[catId]` map with second-update rearm + explicit `collapseCategory`/`toggleCategory`-to-collapse cancels pending timer + per-category isolation (updates to readiness do not touch relationships's timer) + retract preserves `revealed` sticky gate, `_getPendingRetractCount([catId])` harness-only introspection exposed for deterministic verification, `verify-phase5.js` 30/30 harness green across 9 groups (G1 config + init state, G2 unexpanded-cat no-op, G3 grace window suppression, G4 past-window schedule + fire + reveal-preserve, G5 debounce rearm, G6 collapse-cancels, G7 toggle-cancels, G8 per-cat scoped isolation, G9 expand stamping semantics) via fresh-inode mirror `tools/_phase5-cache/_fresh-debrief-feed.js` with fake-clock + fake-setTimeout sandbox swap on Date/setTimeout/clearTimeout, Phases 0–4 + sprint-timer regressions all green post-ship (250/250 total across 7 harness suites). Phases 6–7 still scoped. Companion doc Roadmap B = future DOC-114 JOURNAL_V2 pause-menu detail views. | docs/ |
...
| DOC-110 | [NPC_TOOLING_ROADMAP.md](#doc-110-npc_tooling_roadmapmd) — seven-tool authoring suite: P0 schema ✅, P1 NPC Designer ✅, **P2 Bark Workbench ✅**, P3 Verb-Node Stamper ✅, P4 Archetype Studio, P5 Enemy Hydrator (Stats+DPS+Deck ✅), P6 NPC Sprite Studio, P7 Population Planner. | docs/ | → Wave 3 (NPC Refresh tooling source) |
