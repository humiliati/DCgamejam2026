...
| DOC-116 | [GATE_TAXONOMY.md](#doc-116-gate_taxonomymd) — **Gate taxonomy & resolution pipeline.** Six universal gate types (key, quest, faction, schedule, breakable, composite) with tile-agnostic `gates` map. Three-tier resolution: tile override → edge gate → floor gate → no gate. `override: true` flag, no partial inheritance. Stateful vs stateless evaluation model. Tooling surface: World Designer edge gate editor, BV meta panel gate inspector, CLI `bo set-gate`/`bo set-edge-gate`/`bo validate-gates`. Migration path from `lockedDoors`. Delegation boundaries for engine/UI agents. Depends on DOC-107 (reputation), DOC-105 (blockout refresh). Written 2026-04-17, pending implementation. | docs/ |
