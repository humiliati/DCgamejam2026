...
| **N-layer compositing** | Up to 6 background layers per column via multi-hit DDA, rendered back-to-front (painter's algorithm). Short foreground walls reveal layers behind them. |
| **Procedural textures** | 26 textures at 64×64, generated at init. Brick, stone, wood, metal, door, stair, floor variants. Each tile type maps to a texture ID per biome. |
...
face). `wallX` is the fractional U coordinate along the hit face (0–1).
...
```
ROOF_EAVE_L   = 40   // Left eave — lowest roof point
ROOF_SLOPE_L  = 41   // Left slope — intermediate
ROOF_PEAK     = 42   // Ridge — highest point
ROOF_SLOPE_R  = 43   // Right slope — intermediate
ROOF_EAVE_R   = 44   // Right eave — lowest roof point
```
...
Each is 64×64 procedural, tiling horizontally. The texture wraps across
...
staircase of discrete tiles. At the 64×64 texture scale and the chunky
...
```
AWNING = 45   // Horizontal beam / awning / eave
```
...
itself. Create a `wall_with_eave` texture variant: the top 8–12 rows of
the 64×64 texture show a protruding beam with shadow hatching below it.
...
```
DECK = 47   // Raised platform tile (walkable)
```
...
|---|---|---|---|
| `wall_brick_planter` | `brick_light` | Green planter box at row 40–56 | Under-window flower box |
| `wall_brick_hvac` | `brick_light` | Grey metal unit at row 44–60 | Street-facing AC unit |
| `wall_concrete_vent` | `concrete` | Horizontal slat grate at row 32–44 | Ventilation cover |
| `wall_wood_sign` | `wood_plank` | Colored rectangle at row 8–28 | Shop signage |
| `wall_brick_mailslot` | `brick_light` | Small brass rectangle at row 36–42 | Letter slot |
...
**Option A — New tile types:** `WALL_PLANTER = 48`, `WALL_HVAC = 49`, etc.
...
scale (64×64 textures at 480px viewport), this reads fine for small
...
Each window texture is a 64×64 canvas:

1. Generate the base wall texture (brick, stone, wood) as background
2. Define a rectangular window region (e.g., rows 8–48, cols 16–48)
...
Map to new tile types (`WALL_WINDOW_WARM = 50`, etc.) or use the per-cell
...
**Estimated effort:** 3–5 days focused work + performance validation.
...
This multiplies the per-column work. Every column potentially needs 2–3
...
texture, rows 12–44 might be transparent (the glass area), while 0–11
(lintel) and 45–63 (sill + wall below) are opaque.
...
- **Layer 2 (Wall Decor)** is a prerequisite for Phase 4 if using the
  wall decor data model for per-face fixture placement. Phase 4 here
  sidesteps that by baking fixtures into composite textures — no wall
  decor system needed.
...
- Promenade (floor "1") and Lantern Row (floor "2") are the primary
  beneficiaries of Phases 1–5. Their building facades face the player
...
All new tile types in Phases 1–3 must be registered in all three contract
...
Current highest tile type: DETRITUS = 39. Phases 1–4 propose tile types
40–50 (11 new IDs). JavaScript has no practical tile ID limit (plain
object key lookup), but the grid uses byte values in floor JSON. Staying
under 256 is safe for the foreseeable future.

New tile types proposed:

| ID | Name | Phase | Walkable | Opaque |
|---|---|---|---|---|
| 40 | ROOF_EAVE_L | 1 | No | Yes |
| 41 | ROOF_SLOPE_L | 1 | No | Yes |
| 42 | ROOF_PEAK | 1 | No | Yes |
| 43 | ROOF_SLOPE_R | 1 | No | Yes |
| 44 | ROOF_EAVE_R | 1 | No | Yes |
| 45 | AWNING | 2 | No | Yes |
| 46 | STOOP | 3 | Yes | No |
| 47 | DECK | 3 | Yes | No |
| 48 | WALL_PLANTER | 4 | No | Yes |
| 49 | WALL_HVAC | 4 | No | Yes |
| 50 | WALL_WINDOW_WARM | 5 | No | Yes |

Additional window variants (WALL_WINDOW_DARK, WALL_WINDOW_SKY, etc.) take
IDs 51+. The exact set depends on how many distinct window types the art
direction requires.
