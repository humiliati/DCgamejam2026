...
#### M2.3 — Per-floor diffs (~¾–1 day)

Partial progress: **M2.3a ✅** (2026-04-15) — CleaningSystem blood map + GrimeGrid sub-tile state (base64 Uint8Array per allocated tile); **M2.3c ✅** (2026-04-14) — Minimap `explored` bitmap + CrateSystem `containers`. Remaining categories listed in `save-state.js:_emptyFloorDiff` as stubbed fields. Order each subslice by risk: tile-level first (cleaned/blood), then entity (enemies, vermin), then structural (traps, doors, buttons, puzzles, chest phases), then reanimated formidables.
