...
console.log('\nExtracting string index from data/strings/en.js...');
try {
  // Register a capturing stub for i18n.register and evaluate the file.
  // The strings file just calls i18n.register('en', {...}) at top-level.
  var captured = {};
  sandbox.i18n = {
    register: function(locale, dict) {
      if (locale === 'en' && dict && typeof dict === 'object') {
        Object.keys(dict).forEach(function(k){ captured[k] = dict[k]; });
      }
    },
    t: function(k){ return k; }
  };
  var stringsSrc = fs.readFileSync(path.join(ROOT, 'data/strings/en.js'), 'utf8');
  vm.runInContext(stringsSrc, sandbox, { filename: 'data/strings/en.js', timeout: 5000 });
  var keys = Object.keys(captured);
  // Bucket by namespace prefix (first token before '.') for the visualizer
  // so it can show "hud.*" as a group without parsing at view time.
  var byNamespace = {};
  keys.forEach(function(k) {
    var ns = k.indexOf('.') >= 0 ? k.split('.')[0] : '_';
    (byNamespace[ns] = byNamespace[ns] || []).push(k);
  });
  fs.writeFileSync(path.join(__dirname, 'string-index.json'), JSON.stringify({
    generated: new Date().toISOString(),
    source: 'data/strings/en.js',
    stringCount: keys.length,
    namespaces: Object.keys(byNamespace).sort(),
    strings: captured
  }, null, 2));
  console.log('  OK: ' + keys.length + ' strings (' + Object.keys(byNamespace).length + ' namespaces) -> tools/string-index.json');
} catch (e) {
  console.warn('  WARN: string index failed - ' + e.message);
}
