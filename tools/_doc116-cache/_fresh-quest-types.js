...
var QuestTypes = (function () {
  ...
  // 'gate-opened'     — advance when a gate tile (KEY/QUEST/FACTION/SCHEDULE/
  //                     BREAKABLE/COMPOSITE) is opened (DOC-116 gate taxonomy
  //                     coordination). Predicate fields: floorId (optional —
  //                     exact match), x (optional), y (optional), gateType?
  //                     ('key'|'quest'|'faction'|'schedule'|'breakable'|
  //                     'composite'). Omit all three of floorId/x/y to match
  //                     any gate; omit gateType to match any gate category.
  //                     Dispatched by QuestChain.onGateOpened(floorId, x, y,
  //                     gateType) wired from the gate-resolution pipeline.
