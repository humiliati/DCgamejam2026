#!/usr/bin/env node
/**
 * DOC-109 Phase 2 — DebriefFeed category wrapper verification (fresh).
 *
 * Mirror of verify-phase2.js written to a new path this session so the
 * sandbox bindfs mount serves its pages hot. Loads engine/debrief-feed.js
 * via a fresh-inode redirect to _fresh-debrief-feed.js (also written
 * this session) so the category-wrapper source is actually visible to
 * Node inside the sandbox.
 *
 *   G1 — initial shape (both categories + API surface)
 *   G2 — legacy expandFaction routes through _categories.relationships
 *   G3 — updateFaction sequence tracks mostRecentId + animation flags
 *   G4 — collapsed vs expanded render DOM + chevron glyph flip
 *   G5 — toggle / collapse / reveal semantics
 *
 * Usage: node tools/_debrief-categories-cache/run-phase2.js
 */
'use strict';
...
function makeSandbox() {
  var ctx = {
    ...
    // AudioSystem, CardAuthority, SessionStats so `typeof X === 'undefined'`
    // guards in debrief-feed.js take the early-return path. Assigning
    // null would flip typeof to 'object' and the guards would fall through
    // and blow up on null.registerZone / null.getAll / etc.
  };
  ctx.window = ctx;
  ctx.global = ctx;
...
(function g4() {
  ...
  assert('G4', 'collapsed chevron is U+25B8 (right-pointing)',
  ...
  assert('G4', 'expanded chevron is U+25BE (down-pointing)',
